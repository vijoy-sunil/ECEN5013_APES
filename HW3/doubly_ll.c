...
node_t* insert_at_beginning(node_t* head, char data)
{
...
node_t* insert_at_end(node_t* head, char data)
{
	//if no head exists, init this node as head
	if(head == NULL)
	{
...
node_t* insert_at_position(node_t* head, char data, int32_t index)
{
	size_t len;
	int i = 0;
	//if no head exists, init this node as head
...
void print_mylist(node_t* head)
{
	...
	if(temp_node == NULL)
		...
		while(temp_node != NULL)
		{
			...
			printf("Element [%d]: %c %d\n", i, curr_node_addr->data, curr_node_addr->count);	
			i++;	
			temp_node = temp_node->next;
		}
